...
run_test "Execução sem argumentos" "./gbv"
run_test "Execução com poucos argumentos" "./gbv -a"
run_test "Execução com opção inválida" "./gbv -x $LIB_NAME"

# 2. Adição de Arquivos (-a)
run_test "Adicionar um arquivo pela primeira vez (cria a biblioteca)" "./gbv -a $LIB_NAME arquivo1.txt"
run_test "Listar conteúdo após primeira adição" "./gbv -l $LIB_NAME"
run_test "Adicionar múltiplos arquivos de uma vez" "./gbv -a $LIB_NAME arquivo2.txt arquivo3.txt"
run_test "Listar conteúdo após múltiplas adições" "./gbv -l $LIB_NAME"
run_test "Adicionar um arquivo que não existe" "./gbv -a $LIB_NAME arquivo_inexistente.txt"
run_test "Adicionar um arquivo vazio" "./gbv -a $LIB_NAME vazio.txt"
run_test "Listar para verificar arquivo vazio" "./gbv -l $LIB_NAME"

# 3. Substituição de Arquivo
run_test "Substituir um arquivo existente (adicionar novamente)" "./gbv -a $LIB_NAME arquivo1.txt"
echo "Verifique se a data de 'arquivo1.txt' foi atualizada na listagem abaixo."
run_test "Listar para verificar a substituição" "./gbv -l $LIB_NAME"

# 4. Remoção de Arquivos (-r)
run_test "Remover um arquivo do meio da lista ('arquivo2.txt')" "./gbv -r $LIB_NAME arquivo2.txt"
run_test "Listar para verificar a remoção" "./gbv -l $LIB_NAME"
run_test "Remover um arquivo que não existe na biblioteca" "./gbv -r $LIB_NAME arquivo_inexistente.txt"
run_test "Adicionar arquivo de volta para próximos testes" "./gbv -a $LIB_NAME arquivo2.txt"
run_test "Remover o primeiro arquivo da lista ('arquivo1.txt')" "./gbv -r $LIB_NAME arquivo1.txt"
run_test "Listar para verificar remoção do primeiro" "./gbv -l $LIB_NAME"
run_test "Remover o último arquivo da lista ('arquivo2.txt')" "./gbv -r $LIB_NAME arquivo2.txt"
run_test "Listar para verificar remoção do último" "./gbv -l $LIB_NAME"
